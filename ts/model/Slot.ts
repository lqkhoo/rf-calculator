...
// VM
import VMSlot = require('../vm/VMSlot');
// Data
...
class Slot extends AbstractSlot implements ISlot {
    ...
    constructor(item: IItem,
        ...
        this.ViewModel = new VMSlot(this);
    ...
    protected _compute_isEffective2FoldSteel = (): boolean => {
        if (! this.Data.Is2foldSteel(this.id()) // Not fold steel
            || this.Index < Slot.UPGRADE_START_IDX) { // Not upgrade slot
            return false;
        }
        for(var i=Slot.UPGRADE_START_IDX; i<this.Index; i++) { // Check if first instance.
    ...
    protected _compute_hasPrecedingOverrider = (): boolean => {
        ...
        if(this.Index >= AbstractSlot.ARRANGE_START_IDX) { return false; }
    ...
    protected _compute_isOverriding = (): boolean => {
        ...
        if (this.id() === 0                             // Can't override with nothing.
            ...
            || this.Index >= Slot.ARRANGE_START_IDX  // Overriding only works from recipe slots.
    ...
    protected _compute_isBeingOverridden = (): boolean => {
        ...
        for(var i=1; i<Slot.ARRANGE_START_IDX; i++) {
    ...
    protected _compute_LightOreCount = (): number => {
        if(this.Index === 0 || this.Index >= Slot.ARRANGE_START_IDX) { // Light ore only works in recipe slots.
            return 0;
    ...
    protected _compute_isApplyingStats = (): boolean => {
        if(this.IsBeingOverridden()) {
            ...
            if(this.Index < Slot.ARRANGE_START_IDX && !this.IsOverriding()) { // Only base item / overrider give stats in recipe slots
                return false;
            }
        }
        return true;
    ...
    protected _compute_diminishingMultiplier = (): number => {
        ...
        if(this.Index < Slot.UPGRADE_START_IDX) {
            return 1;
            ...
            // Case: material
            let numRepeats = 0;
            for(var i=Slot.UPGRADE_START_IDX; i<this.Index; i++) { // 1/2*O(n^2) n_max=9
    ...
    protected override _compute_stat_chargespeed = ():number => {
        ...
        if (ctx[StatVector.KEY_stat_chargespeed] !== undefined) {
            val = ctx[StatVector.KEY_stat_chargespeed];
        }
        return val;
    }
